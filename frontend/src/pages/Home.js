...
function ReviewForm() {
  ...
  const handleSubmit = async () => {
    if (!form.name || !form.review) return alert('Name and review are required!');
    setLoading(true);
  ...
  if (sent) {
    return (
      <div style={{ textAlign: 'center', padding: '40px', background: '#fff', borderRadius: 16, border: '1px solid var(--border)' }}>
        ...
        <h3 style={{ fontFamily: "'Playfair Display',serif", color: 'var(--navy)', marginBottom: 8 }}>Thank You!</h3>
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: 32, border: '1px solid var(--border)' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <label style={rLabelStyle}>Rating *</label>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'center' }}>
            ...
            <div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(201,151,58,0.15)', border: '1px solid rgba(201,151,58,0.3)', borderRadius: 40, padding: '5px 14px', marginBottom: 28 }}>
                ...
                <span style={{ fontSize: 12, color: 'var(--gold-light)', fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase' }}>MP Board · Admissions Open 2026–27</span>
              </div>
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ borderRadius: 20, overflow: 'hidden', aspectRatio: '4/3', border: '1px solid rgba(255,255,255,0.12)', boxShadow: '0 24px 64px rgba(0,0,0,0.4)', background: 'rgba(255,255,255,0.05)' }}>
                <video autoPlay muted loop playsInline controls={false}
                  style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}>
                  <source src={schoolVideo} type="video/mp4" />
                </video>
              </div>
              <div style={{ position: 'absolute', top: -16, right: -16, background: '#fff', borderRadius: 14, padding: '12px 18px', boxShadow: '0 8px 32px rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ position: 'absolute', bottom: -30, left: -16, background: '#fff', borderRadius: 14, padding: '12px 18px', boxShadow: '0 8px 32px rgba(0,0,0,0.2)', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 0 }}>
            {stats.map((s, i) => (
              <div key={i} style={{ textAlign: 'center', padding: '8px 16px', borderRight: i < 3 ? '1px solid rgba(11,31,58,0.2)' : 'none' }}>
                ...
                <div style={{ fontSize: 12, fontWeight: 600, color: 'rgba(11,31,58,0.65)', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 2 }}>{s.label}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 64, alignItems: 'center' }}>
            <FadeIn>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div style={{ gridColumn: 'span 2', borderRadius: 16, overflow: 'hidden', aspectRatio: '16/7' }}>
                  <img src={image1} alt="School Campus" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
                <div style={{ borderRadius: 16, overflow: 'hidden', aspectRatio: '1' }}>
                  <img src={image2} alt="Science Lab" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
                <div style={{ borderRadius: 16, overflow: 'hidden', aspectRatio: '1' }}>
                  <img src={image3} alt="Library" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                </div>
              </div>
            </FadeIn>
            <FadeIn delay={0.15}>
              ...
              {[
                { icon: '🎓', title: 'MP Board Affiliated', text: 'Providing quality education from Nursery to Class 8 under the Madhya Pradesh Board of Secondary Education (MPBSE) standards.' },
      ...
      <section style={{ background: 'var(--light)', padding: '64px 0' }}>
        <div className="section-inner">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
            {[
              { icon: '📢', label: 'Notices',       sub: 'Latest announcements',    path: '/notices',    color: '#B91C1C', bg: '#FEE2E2' },
              { icon: '🖼️', label: 'Gallery',       sub: 'School photos & events',  path: '/gallery',    color: '#2563EB', bg: '#DBEAFE' },
              { icon: '🏟️', label: 'Facilities',    sub: 'Campus & infrastructure', path: '/facilities', color: '#7C3AED', bg: '#EDE9FE' },
              { icon: '💰', label: 'Fee Structure',  sub: 'Session 2026–27',         path: '/fees',       color: '#92400E', bg: '#FEF3C7' },
              { icon: '📞', label: 'Contact Us',     sub: 'Admission enquiry',       path: '/contact',    color: '#059669', bg: '#D1FAE5' },
            ].map((item, i) => (
                ...
                return (
                  <FadeIn key={r.id} delay={i * 0.1}>
                    <div className="card" style={{ padding: 28, position: 'relative', overflow: 'hidden', transition: 'transform 0.2s' }}
                      ...
                      <div style={{ color: 'var(--gold)', fontSize: 16, marginBottom: 14, letterSpacing: 2 }}>
                        {'★'.repeat(r.stars)}{'☆'.repeat(5 - r.stars)}
                      </div>
                      <p style={{ fontSize: 14, color: 'var(--gray)', lineHeight: 1.75, marginBottom: 22 }}>{r.review}</p>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 40, height: 40, borderRadius: '50%', background: av.bg, color: av.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13 }}>
